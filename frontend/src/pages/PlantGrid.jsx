...
export default function PlantGrid({ plants: externalPlants, hideHeader = false }) {
  ...
  // Use external plants if provided, otherwise internal state
  const isControlled = Array.isArray(externalPlants);
  const plants = isControlled ? externalPlants : internalPlants;

  // Total items including AddPlantCard if internal
  ...
  // Determine items to render on the current page
  const start = page * PLANTS_PER_PAGE;
  const end = start + PLANTS_PER_PAGE;
  const pagePlants = plants.slice(start, end);

  // Add AddPlantCard if internal and last page
  const isLastPage = end >= plants.length;
  if (!isControlled && isLastPage) {
    pagePlants.push({ isAddCard: true });
  }
    ...
    <div ref={gridTopRef} style={{ fontFamily: "'Georgia', serif" }}>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: size.gap, justifyContent: "center" }}>
          ...
          {pagePlants.map((plant, idx) =>
            plant.isAddCard ? (
              <AddPlantCard key="add-card" onClick={() => navigate("/plants/add")} />
            ) : (
              <PlantCard
                key={plant.plantID}
                plant={plant}
        ...
        {totalItems > PLANTS_PER_PAGE && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 24, marginBottom: 24 }}>
            ...
            <span style={{ fontSize: 20, color: '#8a9e80', fontFamily: "'Helvetica Neue', sans-serif" }}>
              {page + 1} / {Math.ceil(totalItems / PLANTS_PER_PAGE)}
            </span>
            <button
              onClick={() => {
                setPage(p => p + 1);
              ...
              disabled={(page + 1) * PLANTS_PER_PAGE >= totalItems}
              style={{
                background: 'none', border: 'none', fontSize: 40,
                cursor: (page + 1) * PLANTS_PER_PAGE >= totalItems ? 'default' : 'pointer',
                color: (page + 1) * PLANTS_PER_PAGE >= totalItems ? '#c8d8c0' : '#4a7c59',
